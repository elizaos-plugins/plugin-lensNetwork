...
function isTransferContent(content) {
  const validTypes = typeof content.recipient === "string" && (typeof content.amount === "string" || typeof content.amount === "number");
  if (!validTypes) {
    return false;
  }
  const validAddresses = content.recipient.startsWith("0x") && content.recipient.length === 42;
...
async function setupWallet(lensProvider, ethProvider, key) {
  const wallet = new Wallet(key, lensProvider, ethProvider);
  return wallet;
}
async function transferTokens(wallet, recipientAddress, amount) {
  try {
